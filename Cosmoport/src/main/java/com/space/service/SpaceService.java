...
public class SpaceService {
    ...
    public List<Ship> getFilteredShips(String name,
                                       ...
                                       Double minRating,
                                       Double maxRating) {
        List<Ship> ships = spaceRepo.findAll();
    ...
    public List<Ship> getSortedShips(List<Ship> ships, Integer pageNumber, Integer pageSize, String order) {
        ...
        if (pageSize == null && pageNumber == null)
            return ships.stream().sorted(getComparator(order)).limit(3).collect(Collectors.toList());

        if (pageSize == null)
            return ships.stream().sorted(getComparator(order)).limit(3).collect(Collectors.toList());

        if (pageNumber == null)
            return ships.stream().sorted(getComparator(order)).limit(pageSize).skip(pageSize).collect(Collectors.toList());

        return ships.stream().sorted(getComparator(order)).skip(pageNumber * pageSize).limit(pageSize).collect(Collectors.toList());
    }

    private Comparator<Ship> getComparator(String order) {
    ...
    public Ship createShip(Ship ship) {
        ...
        if (ship.getName().length() > 50 || ship.getName().equals("") || ship.getPlanet().length() > 50
                || ship.getPlanet().equals("") || ship.getSpeed() < 0.01 || ship.getSpeed() > 0.99
    ...
    public Ship getShipById(Long id) {
        if (id == 0)
            throw new BadRequestException();
        return spaceRepo.findAll().stream().filter(ship -> ship.getId().equals(id)).findFirst().orElseThrow(NotFoundException::new);
    }
